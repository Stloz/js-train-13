...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  for (const key of keys) obj.searchParams.set(key, paramsObj);
  return obj.toString();
}
...
function deleteSearchParams(keys, url) {
  ...
  for (const key of keys) objUrl.searchParams.delete(key);
...
function createURLWithParams(params, url) {
  ...
  // Повертаємо нову URL-адресу в рядковому форматі.
  const objUrl = new URL(url);
  for (const key in params) objUrl.searchParams.set(key, params[key]);
  return objUrl.toString();
}
...
function checkURLParameters(url, params) {
  ...
  for (const element of params)
    result[element] = objUrl.searchParams.has(element);
  return result;
}
